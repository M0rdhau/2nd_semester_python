from codecs import decode
from string import ascii_letters  as  letters

c1 = ""

c2 = "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"

c3 = "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"

c4 = ""

c5 = "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"

c6 = ""

c7 = ""

c8 = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"

c9 = ""

c10 = "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"

#One that we need to decrypt
c11 = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"


def hexToInt(hexadec):
    return decode(hexadec, 'hex')

# Convert each to an integer
integerized = [hexToInt(eval('c' + str(i))) for i in range(1, 12)]

#xor each ciphertext with other
cipherPairs = {}
for i in range(len(integerized)):
    for j in range(i + 1, len(integerized)):
        pair = zip(integerized[i], integerized[j])
        cipherPairs[i, j] = [x[0] ^ x[1] for x in pair]

#As advised - we want to know what spaces look like
print(ord(' '))
print(bin(ord(' ')))

#As it turns out, when you xor something with 32 - you change the case(they're 32 chars apart)
alphabet = [ord(' ') ^ ord(x) for x in letters]
print(letters)
print(''.join(map(chr, alphabet)))
#yep, that's correct
#it follows that every time you xor something with a space - you get an ascii character
#we can check which characters appear how many times - so that we look for a key

keyBytes = {}

for k in cipherPairs:
    for i in range(len(cipherPairs[k])):
        if chr(cipherPairs[k][i]) in letters:
            if i in keyBytes:
                keyBytes[i].extend([integerized[k[0]][i] ^ 32, integerized[k[1]][i] ^ 32])
            else:
                keyBytes[i] = [integerized[k[0]][i] ^ 32, integerized[k[1]][i] ^ 32]

print(keyBytes[0], len(keyBytes[0]))
print(keyBytes[1], len(keyBytes[1]))
print(keyBytes[2], len(keyBytes[2]))
for i in set(keyBytes[2]):
    print(i, keyBytes[2].count(i))


def common_bytes(bytes):
    byteAsSet = set(bytes)
    m = max([bytes.count(x) for x in byteAsSet])
    return [x for x in byteAsSet if bytes.count(x) == m]


bytePlaces = dict([(i, common_bytes(keyBytes[i])) for i in keyBytes])

print(bytePlaces)

for j in range(11):
    partialDecoded = []
    for i in range(len(integerized[j])):
        if i in bytePlaces:
            partialDecoded.append(chr(integerized[j][i] ^ bytePlaces[i][0]))
        else:
            partialDecoded.append('*')
    print(''.join(partialDecoded))

q = "We can factor the number 15 with quantum computers. We can also factor the number 15 with a dog trained to bark three times -- Robert Harley"

key = [ord(q[i]) ^ integerized[0][i] for i in range(len(integerized[0]))]
flag = [key[i] ^ integerized[10][i] for i in range(len(integerized[10]))]

print(''.join(map(chr, flag)))
